...
const dummyEditor: DummyEditor = new DummyEditor();
const dummyGWTEditor: DummyGWTEditor = new DummyGWTEditor(dummyEditor);
const editorId:string = "dummy editor";
const appFormerGwtApi: AppFormerGwtApi = new AppFormerGwtApi();
...
describe("AppFormerGwtApi", () => {
  ...
  test("get non existing editor", () => {
    expect(() => appFormerGwtApi.getEditor("X")).toThrowError("GwtEditor with id 'X' was not found");
  });
});